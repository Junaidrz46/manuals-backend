db.materials.insertMany(
    [
        {   "_id" : ObjectId("5bf9bc9db5a1b0030083d1b8"),
            "name": "iphone8PDF",
            "description": "iphone8 PDF file",
            "fileName": "tenta.pdf",
            "fileDownloadUri": "http://localhost:8888/rest/file/downloadFile/tenta.pdf",
            "fileType": "application/pdf",
            "size": 261456


        },

        {
            "_id" : ObjectId("5bf9b3d0b5a1b0030083cdb3"),
            "name": "iphone8Image",
            "description": "iphone8 PDF image",
            "fileName": "tenta.pdf",
            "fileDownloadUri": "http://localhost:8888/rest/file/downloadFile/tenta.pdf",
            "fileType": "application/pdf",
            "size": 261456


        },

        {

            "_id" : ObjectId("5bf9b4e2b5a1b0030083ce2e"),
            "name": "HW_Y6_2018PDF",
            "description": "HUAWEI Y6 2018 PDF file",
            "fileName": "tenta.pdf",
            "fileDownloadUri": "http://localhost:8888/rest/file/downloadFile/tenta.pdf",
            "fileType": "application/pdf",
            "size": 261456


        },
        {

            "_id" : ObjectId("5bf9b4e2b5a1b0030083ce2f"),
            "name": "HW_Y6_2018Image",
            "description": "HUAWEI Y6 2018 Image file",
            "fileName": "tenta.pdf",
            "fileDownloadUri": "http://localhost:8888/rest/file/downloadFile/tenta.pdf",
            "fileType": "application/pdf",
            "size": 261456


        },



        {

            "_id" : ObjectId("5bf9b665b5a1b0030083cf16"),

            "name": "HW_M20_PROPDF",
            "description": " PDF file",
            "fileName": "tenta.pdf",
            "fileDownloadUri": "http://localhost:8888/rest/file/downloadFile/tenta.pdf",
            "fileType": "application/pdf",
            "size": 261456


        },

        {

            "_id" : ObjectId("5bf9b6e2b5a1b0030083cf45"),

            "name": "LFXC22596SPDF",
            "description": "LFXC22596S PDF file",
            "fileName": "tenta.pdf",
            "fileDownloadUri":
                "http://localhost:8888/rest/file/downloadFile/tenta.pdf",
            "fileType": "application/pdf",
            "size": 261456

        },

        {

            "_id" : ObjectId("5bf9b71cb5a1b0030083cf62"),

            "name": "RF28N9780SG",
            "description": "RF28N9780SG PDF file",
            "fileName": "tenta.pdf",
            "fileDownloadUri":
                "http://localhost:8888/rest/file/downloadFile/tenta.pdf",
            "fileType": "application/pdf",
            "size": 261456

        }
    ]
);